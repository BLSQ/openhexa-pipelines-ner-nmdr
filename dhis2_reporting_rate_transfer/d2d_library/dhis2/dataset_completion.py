...
class DatasetCompletionSync:
    ...
    def sync(
        ...
        parent_ou: [OPTIONAL] List of parent organisation unit IDs to build completion table (if None, no table built).
        period: The period for which to sync the completion status.        
        logging_interval: Interval for logging progress (defaults to 2000).        
        ds_processed_path: [OPTIONAL] Path to save processed org units (if None, no file saving nor comparison).
        """
