...
def log_invalid_org_units(
    ...
    """Logs invalid organisation units found in the error list."""
...
def get_formatted_period(period_back: int = 0, frequency: str = "MONTHLY") -> str:
    """Get formatted period string based on frequency and period window.

    Returns
    -------
        str: Formatted period string (e.g., '202407' for July 202
    """
    if frequency == "WEEKLY":
        # return (datetime.now() - relativedelta(weeks=period_back)).strftime("%YW%W")
        target_date = datetime.now() - relativedelta(weeks=period_back)
        iso_year, iso_week, _ = target_date.isocalendar()
        return f"{iso_year}W{iso_week}"

    if frequency == "MONTHLY":
        return (datetime.now() - relativedelta(months=period_back)).strftime("%Y%m")
    if frequency == "QUARTERLY":
        q_date = datetime.now() - relativedelta(months=period_back * 3)
        quarter = (q_date.month - 1) // 3 + 1
        return f"{q_date.year}Q{quarter}"
    if frequency == "YEARLY":
        return (datetime.now() - relativedelta(years=period_back)).strftime("%Y")
    raise ValueError(f"Unsupported frequency: {frequency}")


def get_extract_periods(start: str, end: str, period_window: int = 0, frequency: str = "MONTHLY") -> list[str]:
    """Compute the list of DHIS2 periods based on pipeline configuration.

    This function expects period strings in DHIS2 format. This formats must be compatible with the
     selected frequency and with the `period_from_string` function from the `openhexa.toolbox.dhis2.periods` module.

    TODO: Make this function to returns different types of periods: weekly, monthly, quarterly, yearly.

    Args:
        start (str): Start period string (e.g., '202407' for July 2024).
        end (str): End period string (e.g., '202409' for September 2024).
        period_window (int, optional): Number of periods back from the current period if start is not provided.
                                        Defaults to 0.
        frequency (str, optional): Frequency of periods ('MONTHLY', 'WEEKLY', etc.). Defaults to 'MONTHLY'.
    ...
    try:
        ...
        raise Exception(f"Error computing extract periods (check period format and frequency match!): {e}") from e
